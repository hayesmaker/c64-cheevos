...
class Stix {
  ...
  newGameCheck() {
    if (parseInt(this.cpuReadNS(MEM_RESET).toString(16), 10) === 0) {
      // this.hasReset = false;
      // return false;
    }
    if (parseInt(this.cpuReadNS(MEM_RESET).toString(16), 10) === 1) {
      // this.hasReset = true;
      return !this.getIsDead() && this.getLives() === 3
    }
    return this.isGameOver && !this.getIsDead() && this.getLives() === 3;
  }
  ...
  execute () {
    ...
    if (currentLives !== this.lives) {
      ...
      console.log('Stix.lives=', this.lives);
    }
    if (currentLives === 0 && this.getIsDead() && !this.isGameOver) {
      ...
      this.postScore(
        ...
        this.watcher.dispatch('cheevo', {
          ...
          message: `Your score of ${this.score} has been submitted to the STIX Leaderboard!`
        });
      });
    }
